...
const ELEMENT_FIELDS: {
  ...
  { key: "imagePosition", label: "Image", color: "#3b82f6" },
  { key: "iconPosition", label: "Icons", color: "#10b981" },
];
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {activeLayout && editorMode === "closed" && (
        <div className="mx-3 mt-3 flex items-center justify-between bg-indigo-50 rounded-xl px-3 py-2 border border-indigo-100">
          ...
          <button
            ...
            className="text-[10px] text-indigo-400 hover:text-red-500 transition-colors font-medium shrink-0"
          >
            Clear
          </button>
        </div>
        ...
        {editorMode !== "closed" ? (
          ...
          <div className="animate-in fade-in slide-in-from-right-2 duration-300">
            <div className="flex items-center justify-between gap-2 mb-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-bold text-gray-800">
                  {editorMode === "editing" ? "Edit Layout" : "Create Layout"}
                </h4>
              </div>
              <button
                onClick={saveCustomLayout}
                disabled={!editorTitle.trim()}
                className="px-3 py-1.5 rounded-lg text-[10px] font-semibold text-white bg-indigo-600 hover:bg-indigo-700 shadow-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
              >
                <LuSave className="w-3 h-3" />
                {editorMode === "editing" ? "Update" : "Save"}
              </button>
            </div>

            {/* Preview */}
            <div className="rounded-xl border-2 border-gray-200 bg-white p-3 flex items-center justify-center shadow-sm mb-3">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {ELEMENT_FIELDS.map((f) => (
                <span key={f.key} className="flex items-center gap-1 text-[9px] text-gray-500">
            ...
            <div className="space-y-2.5">
              <div>
                <label className="text-[10px] font-semibold text-gray-600 mb-0.5 block">Layout Title</label>
                <input
                  type="text"
                  value={editorTitle}
                  onChange={(e) => setEditorTitle(e.target.value)}
                  placeholder="My Custom Layout"
                  className="w-full text-xs px-2.5 py-2 rounded-lg border border-gray-200 bg-white text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-400/30 focus:border-indigo-400 transition-colors"
                />
              </div>
              {ELEMENT_FIELDS.map((field) => (
                <div key={field.key}>
                  ...
                  <select
                    ...
                    className="w-full text-[10px] px-2.5 py-2 rounded-lg border border-gray-200 bg-white text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-400/30 focus:border-indigo-400 transition-colors"
                  >
              ...
              <button
                onClick={cancelEditor}
                className="w-full px-3 py-2 rounded-lg text-[10px] font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 transition-colors"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="space-y-3 animate-in fade-in duration-200">
            <button
              ...
              className="w-full flex items-center gap-3 p-3 rounded-xl border-2 border-dashed border-gray-300 text-left hover:border-indigo-400 hover:bg-indigo-50/30 transition-all duration-200 group"
            >
              <div className="w-10 h-10 rounded-lg bg-indigo-100 flex items-center justify-center group-hover:bg-indigo-200 transition-colors shrink-0">
                <LuPlus className="w-4 h-4 text-indigo-600" />
              </div>
              <div>
                <span className="text-xs font-bold text-gray-700 group-hover:text-indigo-600 transition-colors block">
                  Create New Layout
                </span>
            ...
            {customLayouts.length === 0 ? (
              <div className="text-center py-8">
                <div className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center mx-auto mb-2">
                  <LuLayoutGrid className="w-5 h-5 text-gray-400" />
                </div>
                <p className="text-xs font-semibold text-gray-500">No custom layouts yet</p>
                <p className="text-[10px] text-gray-400 mt-0.5">Create your first layout to get started</p>
              </div>
            ) : (
                ...
                {customLayouts.map((layout) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group relative flex flex-col rounded-xl border-2 p-2 transition-all duration-200 cursor-pointer hover:shadow-lg ${
                        isSelected
                    ...
                    >
                      {isSelected && (
                        <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-indigo-600 flex items-center justify-center shadow-sm z-10">
                      ...
                      <div className="absolute top-1.5 left-1.5 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                        <button
                          ...
                          className="w-5 h-5 rounded-md bg-white/90 border border-gray-200 flex items-center justify-center hover:bg-indigo-50 hover:border-indigo-300 transition-colors"
                          title="Edit layout"
                        >
                          <LuPencil className="w-2.5 h-2.5 text-gray-500" />
                        </button>
                        <button
                          onClick={(e) => { e.stopPropagation(); duplicateCustomLayout(layout); }}
                          className="w-5 h-5 rounded-md bg-white/90 border border-gray-200 flex items-center justify-center hover:bg-blue-50 hover:border-blue-300 transition-colors"
                          title="Duplicate layout"
                        >
                          <LuCopy className="w-2.5 h-2.5 text-gray-500" />
                        </button>
                        <button
                          onClick={(e) => { e.stopPropagation(); deleteCustomLayout(layout.id); }}
                          className="w-5 h-5 rounded-md bg-white/90 border border-gray-200 flex items-center justify-center hover:bg-red-50 hover:border-red-300 transition-colors"
                          title="Delete layout"
                        >
                          <LuTrash2 className="w-2.5 h-2.5 text-gray-500" />
                        </button>
                      </div>

                      <div className="aspect-[4/3] w-full rounded-lg overflow-hidden bg-gray-50 border border-gray-100 mb-1.5">
                        <LayoutPreview layout={layout} />
                      </div>
                      <span className={`text-[10px] font-semibold leading-tight ${isSelected ? "text-indigo-700" : "text-gray-700"}`}>
                        {layout.title ?? layout.name}
                      </span>
                      <span className="text-[8px] text-gray-400 leading-snug mt-0.5 line-clamp-2">
                        {layout.description}
                      </span>
                    </div>
                  );
              ...
              {DEFAULT_LAYOUTS.map((layout) => {
                ...
                return (
                  <button
                    ...
                    className={`group relative flex flex-col rounded-xl border-2 p-2 transition-all duration-200 text-left hover:shadow-lg ${
                      isSelected
                        ? "border-indigo-500 bg-gradient-to-br from-indigo-50/80 to-purple-50/40 shadow-lg ring-2 ring-indigo-200"
                        : "border-gray-200 bg-white hover:border-indigo-300 hover:bg-gray-50"
                    }`}
