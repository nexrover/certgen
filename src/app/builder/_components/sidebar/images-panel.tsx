...
import { LuUpload, LuEllipsis, LuTrash2, LuDownload, LuInfo, LuChevronLeft } from "react-icons/lu";
          ...
          <div className="grid grid-cols-2 gap-2">
            {DEFAULT_IMAGES.map((img, i) => (
              ...
              >
                ...
                <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent px-1.5 py-1 text-[9px] font-medium text-white opacity-0 transition-opacity group-hover:opacity-100 truncate">
                  {img.name}
                </span>
              </button>
            ))}
          ...
          <div className="space-y-3">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 gap-2">
                {images.map((img, i) => (
                  <div key={i} className={`group relative ${activeMenuIndex === i ? 'z-50' : 'z-10'}`}>
                    ...
                    {activeMenuIndex === i && (
                      <div
                        className={`absolute ${i % 2 === 0 ? 'left-0' : 'right-0'} top-8 z-[9999] w-56 rounded-xl bg-white shadow-[0_4px_20px_rgb(0,0,0,0.15)] overflow-hidden text-left`}
                        ...
                        {!showDetails ? (
                          <>
                            <div className="px-3 py-2.5">
                              <div className="flex items-center justify-between">
                                <h4 className="text-xs font-semibold text-gray-900 truncate pr-2">{img.name}</h4>
                              </div>
                            ...
                            <div className="border-t border-gray-100 py-1">
                              ...
                              <button onClick={() => { handleDownload(img.url, img.name); setActiveMenuIndex(null); setShowDetails(false); }} className="flex w-full items-center px-3 py-2 text-xs text-gray-700 hover:bg-gray-50">
                          ...
                          <>
                            <div className="px-3 py-2.5 border-b border-gray-100 flex items-center bg-gray-50/50">
                              ...
                              <h4 className="text-xs font-semibold text-gray-900">Image Details</h4>
                            </div>
                            <div className="px-3 py-3 space-y-3 text-xs text-gray-700">
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                  <span className="block text-[10px] font-medium text-gray-500 uppercase tracking-wider mb-0.5">Format</span>
                              ...
                              <div>
                                <span className="block text-[10px] font-medium text-gray-500 uppercase tracking-wider mb-0.5">Uploaded</span>
              ...
              <div className="text-center py-8">
                <p className="text-xs font-semibold text-gray-500">No uploaded images yet</p>
                <p className="text-[10px] text-gray-400 mt-1">Upload images to use them in your designs</p>
              </div>
            )}
          </div>
        )}
