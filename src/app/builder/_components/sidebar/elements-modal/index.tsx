...
import { LuX, LuUpload, LuEllipsis, LuInfo, LuDownload, LuTrash2, LuChevronLeft } from "react-icons/lu";
...
export function ElementsModal({ isOpen, onClose, canvas }: ElementsModalProps) {
      ...
      <div className="bg-white w-full max-w-5xl h-[80vh] min-h-[600px] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 border border-gray-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/80">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Browse and add design elements to your canvas</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {activeTab === "default" ? (
            <div className="px-6 py-4">
              {/* Category Filter */}
              <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar mb-4" style={{ scrollbarWidth: "none" }}>
            ...
            <div className="px-6 py-4 space-y-4">
              <label className="flex cursor-pointer items-center justify-center rounded-xl border-2 border-dashed border-gray-300 p-6 text-sm text-gray-500 transition-colors hover:border-indigo-400 hover:text-indigo-600">
                ...
                Upload SVG or Image Element
                <input type="file" accept="image/svg+xml,image/png,image/jpeg" className="hidden" onChange={handleUploadCustom} />
              </label>

              {customElements.length === 0 ? (
                <div className="text-center py-12">
                  <p className="text-sm font-semibold text-gray-500">No custom elements yet</p>
                  <p className="text-xs text-gray-400 mt-1">Upload SVGs or images to use as elements</p>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-2">
